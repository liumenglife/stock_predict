...
class RNN(ModelBase):
    ...
    def __init__(self, hparams):
        ...
        self.pred, self.accuracy, self.correct_count, self.precision, self.recall, self.f1, self.n_accuracy,\
            self.true_false = self.evaluate(self.mode, self.logits, self.y, batch_size)
    ...
    def evaluate(self, mode, logits, y, batch_size):
        with tf.name_scope('evaluation'):
            ...
            if mode == 0:
                ...
                # old
                # pred = tf.argmax(logits, 1) # [[up=0.7, down=0.5], [up=0.4, down=0.5]]
                # print(self.pred) # Tensor("evaluation/ArgMax:0", shape=(?,), dtype=int64)
                # print(self.y) # Tensor("y:0", shape=(?, 1), dtype=int64)

                # new
                pred = tf.argmax(logits, axis=2) # [[[up=0.7, down=0.5]], [[up=0.4, down=0.5]]]
                pred = pred[:, -1]
                # print(pred) # Tensor("evaluation/strided_slice:0", shape=(?,), dtype=int64, device=/device:GPU:0)
                pred = tf.reshape(pred, [batch_size, 1])

                # print(y) # Tensor("y:0", shape=(?, 1), dtype=int64, device=/device:GPU:0)
                # print(pred) # Tensor("evaluation/Reshape:0", shape=(?, 1), dtype=int64, device=/device:GPU:0)
